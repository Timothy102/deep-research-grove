...
export const getAllSessionKeys = (sessionId: string) => {
  ...
  return {
    ...
    reportKey: getSessionStorageKey(LOCAL_STORAGE_KEYS.REPORT_DATA_CACHE, sessionId)
  };
};
...
export const saveSessionData = (sessionId: string, data: Record<string, any>) => {
  ...
  try {
    ...
    if (data.report) {
      localStorage.setItem(keys.reportKey, JSON.stringify(data.report));
      localStorage.setItem(LOCAL_STORAGE_KEYS.REPORT_DATA_CACHE, JSON.stringify(data.report));
    }
